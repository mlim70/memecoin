...
// Run a drop with specified number of winners
export const runDrop = async (numWinners: number, walletAddresses: string[]): Promise<DropResult> => {
  try {
    const baseUrl = import.meta.env.VITE_API_BASE_URL || '/api';
    const response = await fetch(`${baseUrl}/run-drop`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to run drop');
  }
};
